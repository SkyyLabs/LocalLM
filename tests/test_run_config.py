...
def test_workflow_config_validates_required_file() -> None:
    try:
        ...
        assert "requires a file" in str(exc)
    else:
        raise AssertionError("Expected summarize without file to fail.")
...
def test_load_workflow_config_from_json(tmp_path: Path, monkeypatch) -> None:
    ...
    config_file.write_text(
        '{"task": "ask", "file": "data/private/doc.txt", "question": "What is this?"}',
        encoding="utf-8",
    )
    ...
    assert workflow.file == Path("data/private/doc.txt")
